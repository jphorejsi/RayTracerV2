...
Vec3 normalMapping(const SceneType& scene, const std::string objectType, const int objectNumber, Vec3& intersectionPoint) {
    ...
    if (objectType == "Sphere") {
        ...
        Vec2 TC2 = scene.textureCordinates[triangle.vt2Index - 1]; //changed from .v2Index
        ...
        T = T.normal(); // Ensure T is normalized
        B = B.normal(); // Ensure B is normalized

        Vec3 newN = (T * m.x + B * m.y + N * m.z).normal(); // Ensure the result is normalized
        return newN;
